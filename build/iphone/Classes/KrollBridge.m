...
NSString * Canniff_Philip_Project4$ModuleRequireFormat = @"(function(exports){"
...
@implementation Canniff_Philip_Project4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_canniff_philip_project4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Canniff_Philip_Project4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Canniff_Philip_Project4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_canniff_philip_project4 = [[Canniff_Philip_Project4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_canniff_philip_project4];
	
	NSString *_canniff_philip_project4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _canniff_philip_project4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_canniff_philip_project4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Canniff_Philip_Project4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.canniff_philip_project4.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.canniff_philip_project4.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
